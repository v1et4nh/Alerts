...
PICKLE_FILE = '../Data/looksrare_price.pickle'
SLEEP = 30
...
def getETHprice():
    url_eur = "https://api.coingecko.com/api/v3/simple/price?ids=looksrare&vs_currencies=eur%2Cbtc&include_market_cap=true&include_24hr_change=true"
    url_usd = "https://api.coingecko.com/api/v3/simple/price?ids=looksrare&vs_currencies=usd%2Cbtc&include_market_cap=true&include_24hr_change=true"
    data_eur = requests.get(url_eur).json()
    peur = round(data_eur["looksrare"]["eur"], 2)
    peur = format(peur, ",")
    peur_val = float(peur.replace(',', ''))
    data = requests.get(url_usd).json()
    pusd = round(data["looksrare"]["usd"], 2)
    pusd = format(pusd, ",")
    pusd_val = float(pusd.replace(',', ''))
    pbtc = round(data["looksrare"]["btc"], 8)
    pchange = round(data["looksrare"]["usd_24h_change"], 2)
...
def run():
    ...
    print(priceinfo)
    if abs(last_eur - eur) >= 0.1:
        message = f"LooksRare: <b>{eur}</b>€\n\n"
        message += priceinfo
