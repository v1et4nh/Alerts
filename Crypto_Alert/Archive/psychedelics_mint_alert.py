...
PICKLE_FILE = '../Data/psychedelics_last_counter.pickle'
ADDRESS     = '0x75e95ba5997eb235f40ecf8347cdb11f18ff640b'
OPENSEA     = 'psychedelics-anonymous-genesis'
SLEEP       = 0.5
...
def getMintedAmount(dict_data):
    ...
    """
    url = 'https://api.etherscan.io/api?module=proxy&action=eth_call&to='+dict_data['address']+'&data=0x9f181b5e&apikey='+dict_data['key']
    data = getData(url)
...
def getMaxSupply(dict_data):
    url = 'https://api.etherscan.io/api?module=proxy&action=eth_call&to='+dict_data['address']+'&data=0xcc5c095c&apikey='+dict_data['key']
    data = getData(url)
...
def run_mint_counter():
    ...
    if mint_counter - last_counter > 0:
        ...
        telegram_bot_sendtext(message, bot_chatID='-1001527992210', disable_web_page_preview=True)
